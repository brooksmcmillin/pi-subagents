...
import { clearStructuredOutputCaptures, MISSING_STRUCTURED_OUTPUT_CALL_ERROR, readStructuredOutput, readStructuredOutputAcceptanceReport, serializeStructuredOutput } from "../shared/structured-output.ts";
...
async function runSingleAttempt(
	...
	if (completionEvidence.legacyFailureError) {
		...
		if (options.outputPath && result.exitCode === 0) {
			const validatedStructuredOutput = result.structuredOutput !== undefined;
			const outputForPersistence = validatedStructuredOutput
				? serializeStructuredOutput(result.structuredOutput)
				: fullOutput;
			const resolvedOutput = resolveSingleOutput(options.outputPath, outputForPersistence, shared.outputSnapshot, {
				authoritative: validatedStructuredOutput,
				expectedClaimPath: options.outputClaimPath,
			});
			...
			if (validatedStructuredOutput && (!resolvedOutput.savedPath || resolvedOutput.saveError)) {
				...
				progress.status = "failed";
				progress.error = result.error;
			} else if (resolvedOutput.fatalError) {
				result.exitCode = 1;
...
async function runSyncCompletionInner(
	...
	const acceptanceFileOutput = options.outputPath
		? result.structuredOutput !== undefined
			? { content: serializeStructuredOutput(result.structuredOutput), path: options.outputPath, authoritative: true, durable: result.savedOutputPath !== undefined }
