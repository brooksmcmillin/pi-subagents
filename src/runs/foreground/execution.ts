...
import { MISSING_STRUCTURED_OUTPUT_CALL_ERROR, readStructuredOutput, readStructuredOutputAcceptanceReport, serializeStructuredOutput } from "../shared/structured-output.ts";
...
async function runSingleAttempt(
	...
	const exitCode = await new Promise<number>((resolve) => {
		...
		const updateActivityState = (now: number): boolean => {
			...
			const toolAttentionTarget = progress.activityState === "needs_attention" ? undefined : openToolAttentionTarget(now);
		...
		const processLine = (line: string) => {
			...
			if (evt.type === "tool_execution_start") {
				const toolArgs = evt.args && typeof evt.args === "object" && !Array.isArray(evt.args)
					...
					: {};
				const activeTool = evt.toolName === undefined ? undefined : recordActiveToolCall(evt as { toolCallId?: unknown; toolName: string }, toolArgs, now);
