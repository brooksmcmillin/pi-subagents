...
async function requestWatchdogPermissionFailClosed(request: WatchdogPermissionRequest): Promise<WatchdogPermissionResult> {
	// The watchdog arbiter depends on an optional peer. Keep it out of the
	// ordinary child-runtime load path and fail closed if it is unavailable.
	try {
		const { requestWatchdogPermission } = await import("../../watchdog/permission-arbiter.ts");
		return await requestWatchdogPermission(request);
	} catch (error) {
		return {
			approved: false,
			reason: `Watchdog permission arbiter failed closed: ${error instanceof Error ? error.message : String(error)}`,
			source: "watchdog",
		};
	}
}
...
export function registerPermissionGate(
	...
	requestPermission: (request: WatchdogPermissionRequest) => Promise<WatchdogPermissionResult> = requestWatchdogPermissionFailClosed,
): void {
...
function activateRequiredChildTools(pi: ExtensionAPI, requiredTools: readonly string[] | undefined): void {
	...
	try {
		active = typeof api.getActiveTools === "function" ? api.getActiveTools() : undefined;
	} catch {
		active = undefined;
	}
