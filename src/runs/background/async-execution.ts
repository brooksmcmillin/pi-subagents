...
function resolveJitiCliFromPackageJson(packageJsonPath: string): string | undefined {
	...
	try {
		pkg = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as typeof pkg;
	} catch (error) {
		throw new Error(`Failed to parse jiti package manifest at '${packageJsonPath}'.`, { cause: error });
	}
...
export function executeAsyncChain(
	...
	const deadlineAt = params.timeoutMs === undefined ? undefined : Date.now() + params.timeoutMs;
...
export function executeAsyncSingle(
	...
	const reads = params.reads === undefined ? agentConfig.defaultReads ?? false : params.reads;
	...
	const deadlineAt = params.absoluteDeadlineAt ?? (params.timeoutMs === undefined ? undefined : Date.now() + params.timeoutMs);
