...
import { createStructuredOutputRuntime, MISSING_STRUCTURED_OUTPUT_CALL_ERROR, readStructuredOutput, readStructuredOutputAcceptanceReport, serializeStructuredOutput } from "../shared/structured-output.ts";
...
async function runSingleStepInner(
	...
	while (modelIndex < candidates.length) {
		...
		if (!retryableModelFailure || modelIndex === candidates.length - 1) break;
...
async function runSubagent(
	...
	const timeoutMessage = config.timeoutMs === undefined ? undefined : `Subagent timed out after ${config.timeoutMs}ms.`;
	...
	const disposeControlInbox = watchAsyncControlInbox(asyncDir, {
		...
		onSteer: (request) => {
			const targetStep = request.targetIndex === undefined ? undefined : statusPayload.steps[request.targetIndex];
