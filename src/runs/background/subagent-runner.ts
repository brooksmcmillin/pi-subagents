...
import { clearStructuredOutputCaptures, createStructuredOutputFileCapture, createStructuredOutputRuntime, formatStructuredOutputRejectionError, MISSING_STRUCTURED_OUTPUT_CALL_ERROR, readStructuredOutput, readStructuredOutputAcceptanceReport, serializeStructuredOutput } from "../shared/structured-output.ts";
...
export async function runSingleStepInner(
	...
	const validatedStructuredOutput = finalResult?.structuredOutput !== undefined;
	const outputForPersistence = validatedStructuredOutput
		? serializeStructuredOutput(finalResult?.structuredOutput)
	...
	const acceptanceFileOutput = step.outputPath
		? validatedStructuredOutput
			? { content: outputForPersistence, path: step.outputPath, authoritative: true, durable: resolvedOutput.savedPath !== undefined }
