...
export const SUBAGENT_TOOL_PROMPT_GUIDELINES = [
	'Use subagent only when delegation is needed. If no agent is selected, call { action: "list" } for discovery and choose only executable, non-disabled agents; launch known named agents directly.',
...
• If no agent has been selected, use { action: "list" } for discovery and choose only executable/non-disabled configured agents. Launch known named agents directly; runtime validation fails closed if unavailable.
