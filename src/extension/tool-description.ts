...
EXECUTION:
• When no agent has been selected, use { action: "list" } for discovery and choose only executable/non-disabled agents. Launch known named agents directly; runtime validation fails closed if unavailable.
...
EXECUTE:
• If no agent is selected, call { action:"list" } for discovery and choose only executable/non-disabled agents. Launch known named agents directly; runtime validation fails closed if unavailable.
