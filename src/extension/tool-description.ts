...
export const SUBAGENT_TOOL_PROMPT_GUIDELINES = [
	"Use subagent only when delegation is needed. When no agent is selected, call { action: \"list\" } for discovery and choose only executable, non-disabled agents. Launch known named agents directly; runtime validation fails closed if unavailable.",
...
• When the workflow has not already selected a named agent, use { action: "list" } for discovery and choose only executable/non-disabled agents. Launch known named agents directly; runtime validation fails closed if they are unavailable.
...
EXECUTION:
• When no agent has been selected yet, use { action: "list" } for discovery and choose only executable/non-disabled configured agents. Launch known named agents directly; runtime validation fails closed if they are unavailable.
...
EXECUTE:
• If no agent is selected, use {action:"list"} for discovery and choose only executable/non-disabled agents. Launch known named agents directly; runtime validation fails closed if unavailable.
