...
describe("child tool plan declared tools", () => {
	...
	it("keeps package-backed inspection_shell in the child tool plan", () => {
		const plan = resolvePiLaunchToolPlan({ tools: ["read", "inspection_shell"] });
		assert.deepEqual(plan.declaredBuiltinTools, ["read", "inspection_shell"]);
		assert.deepEqual(plan.effectiveToolAllowlist, ["read", "inspection_shell"]);
		assert.deepEqual(plan.requiredChildTools, ["read", "inspection_shell"]);
	...
	it("composes inspection_shell with extension and coordination tools while honoring restrictions", () => {
		...
		for (const restriction of [
			...
			const restricted = resolvePiLaunchToolPlan({ tools: ["read", "inspection_shell"], ...restriction });
		...
		const denied = resolvePiLaunchToolPlan({ tools, capabilityCeiling: { version: 1, denyExtensions: true, sources: ["test"] } });
