...
describe("main watchdog review adapter", () => {
	...
	it("ignores incomplete registry slots when resolving watchdog fallback candidates", async () => {
		const a = model("mock", "a");
		const b = model("mock", "b");
		const ctx = createCtx({ current: a, models: [a, b] });
		(ctx as any).modelRegistry.getAvailable = () => [undefined, a, null, b];
		...
		const result = await createMainWatchdogReview(ctx, { streamFn })(request(enabledConfig({ model: "mock/a", fallbackModels: ["mock/b"] }), []));
