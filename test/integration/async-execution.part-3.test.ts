...
describe("async execution utilities", { skip: !available ? "pi packages not available" : undefined }, () => {
	...
	it("background structured output overwrites receipt prose and exposes the saved path", { skip: !isAsyncAvailable() ? "jiti not available" : undefined }, async () => {
		...
		assert.equal(child?.savedOutputPath, outputPath, JSON.stringify(payload.results[0], null, 2));
	});
	...
	it("background structured output fails closed when the artifact cannot be saved", { skip: !isAsyncAvailable() ? "jiti not available" : undefined }, async () => {
		...
		assert.match(child?.outputSaveError ?? "", /EEXIST|ENOTDIR|not a directory/i, JSON.stringify(payload, null, 2));
...
`);
	...
	it("background file-only outputSchema runs persist their validated artifact before completion checks", { skip: !isAsyncAvailable() ? "jiti not available" : undefined }, async () => {
		mockPi.onCall({ output: "Non-authoritative completion prose", structuredOutput: { ok: true, note: "file-only" } });
