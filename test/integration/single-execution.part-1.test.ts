...
`, "utf-8");
		...
		for (const [index, script] of scripts.entries()) {
			...
			for (const fileBacked of [false, true]) {
				for (const asyncParams of [{ async: true }, {}]) {
					...
					assert.equal(result.isError, true);
					const payload = JSON.parse(result.content[0]?.text ?? "null") as { ok?: boolean; errors?: Array<{ line?: number }> };
					assert.equal(payload.ok, false);
					assert.equal(typeof payload.errors?.[0]?.line, "number");
					assert.equal(result.details.asyncId, undefined);
	...
	it("rejects actual keyed runs.all access at runtime after advisory validation", { skip: !createSubagentExecutor ? "executor not importable" : undefined }, async () => {
		...
		assert.equal(result.isError, true);
		assert.equal(mockPi.callCount(), 2);
		assert.match(result.content[0]?.text ?? "", /runs\.all resolves to an ordered array/);
		assert.equal(result.details.workflow?.trace.filter(entry => entry.state === "completed").length, 2);
	});
