...
description: |
  ...
  Do not use for one ordinary read-only quick-scout or
  quick-review launch when the registered subagent tool guidance already names
...
For one ordinary read-only quick-scout or quick-review launch, do not load this
skill or its references; follow the registered subagent tool guidance directly.
Load this skill when the role is unfamiliar, the launch is customized, or the
work needs fanout, retries, lifecycle control, mutation, or broader orchestration.
