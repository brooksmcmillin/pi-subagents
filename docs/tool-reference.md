...
File-only outputs are write-once terminal handoffs, not progress files. Finish implementation, Git publication, and verification before writing the output as the sole tool call in its batch. Native children cannot mutate after publication; only inspection and supervisor communication remain available. Output-bound native workers must use follow-up commits and ordinary pushes once published. The conditional guard checks configured upstreams and same-name remote refs; it rejects recognized rewrite commands with target rebinding (`git -C`, Git directory environment/options, or shell directory changes). Apparently unpublished branches retain rewrite support. Publication to differently named destinations without upstream tracking can evade detection, as can shell aliases and dynamic programs. This is a workflow safety guard, not a shell sandbox. External runners receive write-once runtime persistence but do not have native tool-call guards.

A new or resumed file-only run must use an unused output path. On resume, pass the new path explicitly as `output` (and retain `outputMode: "file-only"`); mentioning it only in `task` or `message` cannot override a stored destination. Reusing an existing destination fails before launch. Runtime persistence permits identical-byte replay, never replacement of published bytes.
